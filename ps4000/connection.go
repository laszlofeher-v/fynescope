...
func getValues(m *genericps.GetValuesMsg) {
	...
	numOfSamples, overflow, err = ps4000GetValues(m.Handle(), m.StartIndex,
		m.ReqNumOfSamples,
		m.DownSampleRatio,
...
func setChannel(m *genericps.SetChannelMsg) {
	var err error
	err = ps4000SetChannel(m.Handle(), ChannelId(m.Channel),
...
func flashLed(m *genericps.FlashLedMsg) {
	err := ps4000FlashLed(m.Handle(), m.Start)
...
func getUnitInfo(m *genericps.GetUnitInfoMsg) {
	...
	s, err = ps4000GetUnitInfo(m.Handle(), PicoInfo(m.Info))
	if err != nil {
...
func getValuesAsync(m *genericps.GetValuesAsyncMsg) {
	var err error
	err = ps4000GetValuesAsync(m.Handle(), m.StartIndex,
		m.NumOfSamples,
		m.DownSampleRatio,
		RatioMode(m.DownSampleRatioMode),
...
func getValuesBulk(m *genericps.GetValuesBulkMsg) {
	...
	numOfSamples, err = ps4000GetValuesBulk(m.Handle(), m.ReqNumOfSamples, m.FromSegmentIndex, m.ToSegmentIndex,
...
func getValuesOverlapped(m *genericps.GetValuesOverlappedMsg) {
	...
	numOfSamples, err = ps4000GetValuesOverlapped(m.Handle(), m.StartIndex, m.ReqNumOfSamples,
...
func getValuesOverlappedBulk(m *genericps.GetValuesOverlappedBulkMsg) {
	...
	numOfSamples, err = ps4000GetValuesOverlappedBulk(m.Handle(), m.StartIndex, m.ReqNumOfSamples,
...
func getAnalogueOffsetMsg(m *genericps.GetAnalogueOffsetMsg) {
	...
	maximumVoltage, minimumVoltage, err = ps4000GetAnalogueOffset(m.Handle(), m.VoltageRange, Coupling(m.Coupling))
...
func getChannelInformation(m *genericps.GetChannelInformationMsg) {
	...
	lengthOfRanges, err = ps4000GetChannelInformation(m.Handle(), m.Info, m.Probe, m.Ranges, ChannelId(m.Channel))
...
func getMaxDownSampleRatio(m *genericps.GetMaxDownSampleRatioMsg) {
	...
	maxDownSampleRatio, err = ps4000GetMaxDownSampleRatio(m.Handle(), m.NumOfUnaggregatedSamples, RatioMode(m.DownSampleRatioMode), m.SegmentIndex)
...
func getMaxSegments(m *genericps.GetMaxSegmentsMsg) {
	maxSegments, err := ps4000GetMaxSegments(m.Handle())
...
func getNumberOfCaptures(m *genericps.GetNumOfCapturesMsg) {
	numOfCaptures, err := ps4000GetNumOfCaptures(m.Handle())
...
func getNumberOfProcessedCaptures(m *genericps.GetNumOfProcessedCapturesMsg) {
	numOfCaptures, err := ps4000GetNumOfProcessedCaptures(m.Handle())
...
func getStreamingLatestValues(m *genericps.GetStreamingLatestValuesMsg) {
	...
	err = ps4000GetStreamingLatestValues(m.Handle(), StreamingReady(m.LpStreamingReadyGoPar), m.Param)
...
func getTimebase(m *genericps.GetTimebaseMsg) {
	timeIntervalNanoseconds, maxSamples, err := ps4000GetTimebase(m.Handle(), m.TimeBase, m.NumOfSamples, m.OverSample, m.SegmentIndex)
...
func getTimebase2(m *genericps.GetTimebase2Msg) {
	timeIntervalNanoseconds, maxSamples, err := ps4000GetTimebase2(m.Handle(), m.TimeBase, m.NumOfSamples, m.OverSample, m.SegmentIndex)
...
func maximumValue(m *genericps.MaximumValueMsg) {
	value, err := ps4000MaximumValue(m.Handle())
...
func minimumValue(m *genericps.MinimumValueMsg) {
	value, err := ps4000MinimumValue(m.Handle())
...
func setSimpleTrigger(m *genericps.SetSimpleTriggerMsg) {
	...
	err = ps4000SetSimpleTrigger(m.Handle(), m.Enable, ChannelId(m.Source),
...
func setDataBuffer(m *genericps.SetDataBufferMsg) {
	...
	err = ps4000SetDataBuffer(m.Handle(), ChannelId(m.Ch), m.BufferIn, m.SegmentIndex, RatioMode(m.Mode))
...
func setDataBuffers(m *genericps.SetDataBuffersMsg) {
	...
	err = ps4000SetDataBuffers(m.Handle(), ChannelId(m.Ch), m.BufferMax, m.BufferMin, m.SegmentIndex, RatioMode(m.Mode))
...
func setUnscaledDataBuffers(m *genericps.SetUnscaledDataBuffersMsg) {
	...
	err = ps4000SetUnscaledDataBuffers(m.Handle(), ChannelId(m.Ch), m.BufferMax, m.BufferMin, m.SegmentIndex, RatioMode(m.Mode))
...
func setEtsTimeBuffer(m *genericps.SetEtsTimeBufferMsg) {
	err := ps4000SetEtsTimeBuffer(m.Handle(), m.Buffer)
...
func setEtsTimeBuffers(m *genericps.SetEtsTimeBuffersMsg) {
	err := ps4000SetEtsTimeBuffers(m.Handle(), m.TimeUpper, m.TimeLower)
	response := m.Rsp().(*genericps.SetEtsTimeBufferRsp)
	response.SetStatus(err)
	m.RspCh() <- struct{}{}
}
func setEts(m *genericps.SetEtsMsg) {
	...
	sampleTimePicoseconds, err = ps4000SetEts(m.Handle(), EtsMode(m.Mode), m.EtsCycles, m.EtsInterleave)
...
func runStreaming(m *genericps.RunStreamingMsg) {
	...
	sampleInterval, err = ps4000RunStreaming(m.Handle(), m.ReqSampleInterval, TimeUnits(m.SampleIntervalTimeUnits), m.MaxPostTriggerSamples,
...
func runBlock(m *genericps.RunBlockMsg) {
	...
	timeIndisposedMs, err = ps4000RunBlock(m.Handle(), m.NumOfPreTriggerSamples, m.NumOfPostTriggerSamples,
...
func setTriggerChannelProperties(m *genericps.SetTriggerChannelPropertiesMsg) {
	...
	err = ps4000SetTriggerChannelProperties(m.Handle(), tcp, m.AuxOutputEnable, m.AutoTriggerMs)
...
func setTriggerChannelConditions(m *genericps.SetTriggerChannelConditionsMsg) {
	...
	err = ps4000SetTriggerChannelConditions(m.Handle(), tc)
...
func setTriggerChannelDirections(m *genericps.SetTriggerChannelDirectionsMsg) {
	...
	err = ps4000SetTriggerChannelDirections(m.Handle(), ThresholdDirection(m.ChannelA),
...
func setTriggerDelay(m *genericps.SetTriggerDelayMsg) {
	err := ps4000SetTriggerDelay(m.Handle(), m.Delay)
...
func setPulseWidthQualifier(m *genericps.SetPulseWidthQualifierMsg) {
	...
	err = ps4000SetPulseWidthQualifier(m.Handle(), c, ThresholdDirection(m.Direction), m.Lower, m.Upper, PulseWidthType(m.PwType))
...
func setTriggerDigitalPortProperties(m *genericps.SetTriggerDigitalPortPropertiesMsg) {
	...
	err = ps4000SetTriggerDigitalPortProperties(m.Handle(), tdp)
...
func setSigGenBuiltIn(m *genericps.SetSigGenBuiltInMsg) {
	...
	err = ps4000SetSigGenBuiltIn(m.Handle(), m.OffsetVoltage, m.PkToPK, WaveTypeEnum(m.WaveType), m.StartFrequency,
...
func setSigGenBuiltInV2(m *genericps.SetSigGenBuiltInV2Msg) {
	...
	err = ps4000SetSigGenBuiltInV2(m.Handle(), m.OffsetVoltage, m.PkToPK, WaveTypeEnum(m.WaveType), m.StartFrequency,
		m.StopFrequency, m.Increment, m.DwellTime, SweepTypeEnum(m.SweepType),
		ExtraOperations(m.Operation), m.Shots, m.Sweeps, SigGenTrigType(m.TriggerType),
		SigGenTrigSource(m.TriggerSource), m.ExtInThreshold)
...
func sigGenFrequencyToPhase(m *genericps.SigGenFrequencyToPhasenMsg) {
	...
	phase, err = ps4000SigGenFrequencyToPhase(m.Handle(), m.Frequency, IndexMode(m.IndexMode), m.BufferLength)
...
func setNumOfCaptures(m *genericps.SetNumOfCapturesMsg) {
	err := ps4000SetNoCaptures(m.Handle(), m.NCaptures)
...
func getTriggerTimeOffset(m *genericps.GetTriggerTimeOffsetMsg) {
	...
	timeUpper, timeLower, timeUnits, err = ps4000GetTriggerTimeOffset(m.Handle(), m.SegmentIndex)
...
func getTriggerTimeOffset64(m *genericps.GetTriggerTimeOffset64Msg) {
	time, timeUnits, err := ps4000GetTriggerTimeOffset64(m.Handle(), m.SegmentIndex)
...
func getValuesTriggerTimeOffsetBulk(m *genericps.GetValuesTriggerTimeOffsetBulkMsg) {
	...
	err = ps4000GetValuesTriggerTimeOffsetBulk(m.Handle(), m.TimesUpper, m.TimesLower, tu, m.FromSegmentIndex, m.ToSegmentIndex)
...
func getValuesTriggerTimeOffsetBulk64(m *genericps.GetValuesTriggerTimeOffsetBulk64Msg) {
	...
	err = ps4000GetValuesTriggerTimeOffsetBulk64(m.Handle(), m.Times, tu, m.FromSegmentIndex, m.ToSegmentIndex)
...
func holdOff(m *genericps.HoldOffMsg) {
	err := ps4000HoldOff(m.Handle(), m.HoldOff, HoldOffType(m.HoldOffType))
...
func lsReady(m *genericps.LsReadyMsg) {
	ready, err := ps4000LsReady(m.Handle())
...
func triggerOrPulseWidthQualifierEnabled(m *genericps.TriggerOrPulseWidthQualifierEnabledMsg) {
	triggerEnabled, pulseWidthQualifierEnabledint16, err := ps4000TriggerOrPulseWidthQualifierEnabled(m.Handle())
...
func memorySegments(m *genericps.MemorySegmentsMsg) {
	nMaxSamples, err := ps4000MemorySegments(m.Handle(), m.NSegments)
...
func numOfStreamingValues(m *genericps.NumOfStreamingValuesMsg) {
	numOfValues, err := ps4000NoOfStreamingValues(m.Handle())
...
func queryOutputEdgeDetect(m *genericps.QueryOutputEdgeDetectMsg) {
	state, err := ps4000QueryOutputEdgeDetect(m.Handle())
...
func setDigitalPort(m *genericps.SetDigitalPortMsg) {
	...
	err = ps4000SetDigitalPort(m.Handle(), DigitalPort(m.Port), m.Enabled, m.Logiclevel)
	response := m.Rsp().(*genericps.SetDigitalAnalogTriggerOperandRsp)
	response.SetStatus(err)
	m.RspCh() <- struct{}{}
}
...
func setOutputEdgeDetect(m *genericps.SetOutputEdgeDetectMsg) {
	err := ps4000SetOutputEdgeDetect(m.Handle(), m.State)
...
func setPulseWidthDigitalPortProperties(m *genericps.SetPulseWidthDigitalPortPropertiesMsg) {
	...
	err = ps4000SetPulseWidthDigitalPortProperties(m.Handle(), dd)
...
func setSigGenArbitrary(m *genericps.SetSigGenArbitraryMsg) {
	...
	err = ps4000SetSigGenArbitrary(m.Handle(), m.OffsetVoltage, m.PkToPK, m.StartDeltaPhase,
...
func setSigGenPropertiesArbitrary(m *genericps.SetSigGenPropertiesArbitraryMsg) {
	err := ps4000SetSigGenPropertiesArbitrary(m.Handle(), m.OffsetVoltage, m.StartDeltaPhase,
...
func setSigGenPropertiesBuiltIn(m *genericps.SetSigGenPropertiesBuiltInMsg) {
	err := ps4000SetSigGenPropertiesBuiltIn(m.Handle(), m.OffsetVoltage, float64(m.StartFrequency),
...
func sigGenArbitraryMinMaxValues(m *genericps.SigGenArbitraryMinMaxValuesMsg) {
	minArbitraryWaveformValue, maxArbitraryWaveformValue, minArbitraryWaveformSize,
		maxArbitraryWaveformSize, err := ps4000SigGenArbitraryMinMaxValues(m.Handle())
...
func sigGenSoftwareControl(m *genericps.SigGenSoftwareControlMsg) {
	err := ps4000SigGenSoftwareControl(m.Handle(), m.State)
