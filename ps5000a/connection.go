...
func getValues(m *genericps.GetValuesMsg) {
	...
	numOfSamples, overflow, err = ps5000aGetValues(m.Handle(), m.StartIndex,
		m.ReqNumOfSamples,
		m.DownSampleRatio,
...
func setChannel(m *genericps.SetChannelMsg) {
	var err error
	err = ps5000aSetChannel(m.Handle(), ChannelId(m.Channel),
...
func flashLed(m *genericps.FlashLedMsg) {
	err := ps5000aFlashLed(m.Handle(), m.Start)
...
func getUnitInfo(m *genericps.GetUnitInfoMsg) {
	...
	s, err = ps5000aGetUnitInfo(m.Handle(), PicoInfo(m.Info))
	if err != nil {
...
func getValuesAsync(m *genericps.GetValuesAsyncMsg) {
	var err error
	err = ps5000aGetValuesAsync(m.Handle(), m.StartIndex,
		m.NumOfSamples,
		m.DownSampleRatio,
		RatioMode(m.DownSampleRatioMode),
...
func getValuesBulk(m *genericps.GetValuesBulkMsg) {
	...
	numOfSamples, err = ps5000aGetValuesBulk(m.Handle(), m.ReqNumOfSamples, m.FromSegmentIndex, m.ToSegmentIndex,
...
func getValuesOverlapped(m *genericps.GetValuesOverlappedMsg) {
	...
	numOfSamples, err = ps5000aGetValuesOverlapped(m.Handle(), m.StartIndex, m.ReqNumOfSamples,
...
func getValuesOverlappedBulk(m *genericps.GetValuesOverlappedBulkMsg) {
	...
	numOfSamples, err = ps5000aGetValuesOverlappedBulk(m.Handle(), m.StartIndex, m.ReqNumOfSamples,
...
func getAnalogueOffsetMsg(m *genericps.GetAnalogueOffsetMsg) {
	...
	maximumVoltage, minimumVoltage, err = ps5000aGetAnalogueOffset(m.Handle(), m.VoltageRange, Coupling(m.Coupling))
...
func getChannelInformation(m *genericps.GetChannelInformationMsg) {
	...
	lengthOfRanges, err = ps5000aGetChannelInformation(m.Handle(), m.Info, m.Probe, m.Ranges, ChannelId(m.Channel))
...
func getMaxDownSampleRatio(m *genericps.GetMaxDownSampleRatioMsg) {
	...
	maxDownSampleRatio, err = ps5000aGetMaxDownSampleRatio(m.Handle(), m.NumOfUnaggregatedSamples, RatioMode(m.DownSampleRatioMode), m.SegmentIndex)
...
func getMaxSegments(m *genericps.GetMaxSegmentsMsg) {
	maxSegments, err := ps5000aGetMaxSegments(m.Handle())
...
func getNumberOfCaptures(m *genericps.GetNumOfCapturesMsg) {
	numOfCaptures, err := ps5000aGetNumOfCaptures(m.Handle())
...
func getNumberOfProcessedCaptures(m *genericps.GetNumOfProcessedCapturesMsg) {
	numOfCaptures, err := ps5000aGetNumOfProcessedCaptures(m.Handle())
...
func getStreamingLatestValues(m *genericps.GetStreamingLatestValuesMsg) {
	...
	err = ps5000aGetStreamingLatestValues(m.Handle(), StreamingReady(m.LpStreamingReadyGoPar), m.Param)
...
func getTimebase(m *genericps.GetTimebaseMsg) {
	timeIntervalNanoseconds, maxSamples, err := ps5000aGetTimebase(m.Handle(), m.TimeBase, m.NumOfSamples, m.OverSample, m.SegmentIndex)
...
func getTimebase2(m *genericps.GetTimebase2Msg) {
	timeIntervalNanoseconds, maxSamples, err := ps5000aGetTimebase2(m.Handle(), m.TimeBase, m.NumOfSamples, m.OverSample, m.SegmentIndex)
...
func maximumValue(m *genericps.MaximumValueMsg) {
	value, err := ps5000aMaximumValue(m.Handle())
...
func minimumValue(m *genericps.MinimumValueMsg) {
	value, err := ps5000aMinimumValue(m.Handle())
...
func setSimpleTrigger(m *genericps.SetSimpleTriggerMsg) {
	...
	err = ps5000aSetSimpleTrigger(m.Handle(), m.Enable, ChannelId(m.Source),
...
func setDataBuffer(m *genericps.SetDataBufferMsg) {
	...
	err = ps5000aSetDataBuffer(m.Handle(), ChannelId(m.Ch), m.BufferIn, m.SegmentIndex, RatioMode(m.Mode))
...
func setDataBuffers(m *genericps.SetDataBuffersMsg) {
	...
	err = ps5000aSetDataBuffers(m.Handle(), ChannelId(m.Ch), m.BufferMax, m.BufferMin, m.SegmentIndex, RatioMode(m.Mode))
...
func setUnscaledDataBuffers(m *genericps.SetUnscaledDataBuffersMsg) {
	...
	err = ps5000aSetUnscaledDataBuffers(m.Handle(), ChannelId(m.Ch), m.BufferMax, m.BufferMin, m.SegmentIndex, RatioMode(m.Mode))
...
func setEtsTimeBuffer(m *genericps.SetEtsTimeBufferMsg) {
	err := ps5000aSetEtsTimeBuffer(m.Handle(), m.Buffer)
...
func setEtsTimeBuffers(m *genericps.SetEtsTimeBuffersMsg) {
	err := ps5000aSetEtsTimeBuffers(m.Handle(), m.TimeUpper, m.TimeLower)
	response := m.Rsp().(*genericps.SetEtsTimeBufferRsp)
	response.SetStatus(err)
	m.RspCh() <- struct{}{}
}
func setEts(m *genericps.SetEtsMsg) {
	...
	sampleTimePicoseconds, err = ps5000aSetEts(m.Handle(), EtsMode(m.Mode), m.EtsCycles, m.EtsInterleave)
...
func runStreaming(m *genericps.RunStreamingMsg) {
	...
	sampleInterval, err = ps5000aRunStreaming(m.Handle(), m.ReqSampleInterval, TimeUnits(m.SampleIntervalTimeUnits), m.MaxPostTriggerSamples,
...
func runBlock(m *genericps.RunBlockMsg) {
	...
	timeIndisposedMs, err = ps5000aRunBlock(m.Handle(), m.NumOfPreTriggerSamples, m.NumOfPostTriggerSamples,
...
func setTriggerChannelProperties(m *genericps.SetTriggerChannelPropertiesMsg) {
	...
	err = ps5000aSetTriggerChannelProperties(m.Handle(), tcp, m.AuxOutputEnable, m.AutoTriggerMs)
...
func setTriggerChannelConditions(m *genericps.SetTriggerChannelConditionsMsg) {
	...
	err = ps5000aSetTriggerChannelConditions(m.Handle(), tc)
...
func setTriggerChannelDirections(m *genericps.SetTriggerChannelDirectionsMsg) {
	...
	err = ps5000aSetTriggerChannelDirections(m.Handle(), ThresholdDirection(m.ChannelA),
...
func setTriggerDelay(m *genericps.SetTriggerDelayMsg) {
	err := ps5000aSetTriggerDelay(m.Handle(), m.Delay)
...
func setPulseWidthQualifier(m *genericps.SetPulseWidthQualifierMsg) {
	...
	err = ps5000aSetPulseWidthQualifier(m.Handle(), c, ThresholdDirection(m.Direction), m.Lower, m.Upper, PulseWidthType(m.PwType))
...
func setTriggerDigitalPortProperties(m *genericps.SetTriggerDigitalPortPropertiesMsg) {
	...
	err = ps5000aSetTriggerDigitalPortProperties(m.Handle(), tdp)
...
func setSigGenBuiltIn(m *genericps.SetSigGenBuiltInMsg) {
	...
	err = ps5000aSetSigGenBuiltIn(m.Handle(), m.OffsetVoltage, m.PkToPK, WaveTypeEnum(m.WaveType), m.StartFrequency,
...
func setSigGenBuiltInV2(m *genericps.SetSigGenBuiltInV2Msg) {
	...
	err = ps5000aSetSigGenBuiltInV2(m.Handle(), m.OffsetVoltage, m.PkToPK, WaveTypeEnum(m.WaveType), m.StartFrequency,
		m.StopFrequency, m.Increment, m.DwellTime, SweepTypeEnum(m.SweepType),
		ExtraOperations(m.Operation), m.Shots, m.Sweeps, SigGenTrigType(m.TriggerType),
		SigGenTrigSource(m.TriggerSource), m.ExtInThreshold)
...
func sigGenFrequencyToPhase(m *genericps.SigGenFrequencyToPhasenMsg) {
	...
	phase, err = ps5000aSigGenFrequencyToPhase(m.Handle(), m.Frequency, IndexMode(m.IndexMode), m.BufferLength)
...
func setNumOfCaptures(m *genericps.SetNumOfCapturesMsg) {
	err := ps5000aSetNoCaptures(m.Handle(), m.NCaptures)
...
func getTriggerTimeOffset(m *genericps.GetTriggerTimeOffsetMsg) {
	...
	timeUpper, timeLower, timeUnits, err = ps5000aGetTriggerTimeOffset(m.Handle(), m.SegmentIndex)
...
func getTriggerTimeOffset64(m *genericps.GetTriggerTimeOffset64Msg) {
	time, timeUnits, err := ps5000aGetTriggerTimeOffset64(m.Handle(), m.SegmentIndex)
...
func getValuesTriggerTimeOffsetBulk(m *genericps.GetValuesTriggerTimeOffsetBulkMsg) {
	...
	err = ps5000aGetValuesTriggerTimeOffsetBulk(m.Handle(), m.TimesUpper, m.TimesLower, tu, m.FromSegmentIndex, m.ToSegmentIndex)
...
func getValuesTriggerTimeOffsetBulk64(m *genericps.GetValuesTriggerTimeOffsetBulk64Msg) {
	...
	err = ps5000aGetValuesTriggerTimeOffsetBulk64(m.Handle(), m.Times, tu, m.FromSegmentIndex, m.ToSegmentIndex)
...
func holdOff(m *genericps.HoldOffMsg) {
	err := ps5000aHoldOff(m.Handle(), m.HoldOff, HoldOffType(m.HoldOffType))
...
func lsReady(m *genericps.LsReadyMsg) {
	ready, err := ps5000aLsReady(m.Handle())
...
func triggerOrPulseWidthQualifierEnabled(m *genericps.TriggerOrPulseWidthQualifierEnabledMsg) {
	triggerEnabled, pulseWidthQualifierEnabledint16, err := ps5000aTriggerOrPulseWidthQualifierEnabled(m.Handle())
...
func memorySegments(m *genericps.MemorySegmentsMsg) {
	nMaxSamples, err := ps5000aMemorySegments(m.Handle(), m.NSegments)
...
func numOfStreamingValues(m *genericps.NumOfStreamingValuesMsg) {
	numOfValues, err := ps5000aNoOfStreamingValues(m.Handle())
...
func queryOutputEdgeDetect(m *genericps.QueryOutputEdgeDetectMsg) {
	state, err := ps5000aQueryOutputEdgeDetect(m.Handle())
...
func setDigitalPort(m *genericps.SetDigitalPortMsg) {
	...
	err = ps5000aSetDigitalPort(m.Handle(), DigitalPort(m.Port), m.Enabled, m.Logiclevel)
	response := m.Rsp().(*genericps.SetDigitalAnalogTriggerOperandRsp)
	response.SetStatus(err)
	m.RspCh() <- struct{}{}
}
...
func setOutputEdgeDetect(m *genericps.SetOutputEdgeDetectMsg) {
	err := ps5000aSetOutputEdgeDetect(m.Handle(), m.State)
...
func setPulseWidthDigitalPortProperties(m *genericps.SetPulseWidthDigitalPortPropertiesMsg) {
	...
	err = ps5000aSetPulseWidthDigitalPortProperties(m.Handle(), dd)
...
func setSigGenArbitrary(m *genericps.SetSigGenArbitraryMsg) {
	...
	err = ps5000aSetSigGenArbitrary(m.Handle(), m.OffsetVoltage, m.PkToPK, m.StartDeltaPhase,
...
func setSigGenPropertiesArbitrary(m *genericps.SetSigGenPropertiesArbitraryMsg) {
	err := ps5000aSetSigGenPropertiesArbitrary(m.Handle(), m.OffsetVoltage, m.StartDeltaPhase,
...
func setSigGenPropertiesBuiltIn(m *genericps.SetSigGenPropertiesBuiltInMsg) {
	err := ps5000aSetSigGenPropertiesBuiltIn(m.Handle(), m.OffsetVoltage, float64(m.StartFrequency),
...
func sigGenArbitraryMinMaxValues(m *genericps.SigGenArbitraryMinMaxValuesMsg) {
	minArbitraryWaveformValue, maxArbitraryWaveformValue, minArbitraryWaveformSize,
		maxArbitraryWaveformSize, err := ps5000aSigGenArbitraryMinMaxValues(m.Handle())
...
func sigGenSoftwareControl(m *genericps.SigGenSoftwareControlMsg) {
	err := ps5000aSigGenSoftwareControl(m.Handle(), m.State)
