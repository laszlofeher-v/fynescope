...
func getValues(m *genericps.GetValuesMsg) {
	...
	numOfSamples, overflow, err = ps5000aGetValues(m.Handle(), uint32(m.StartIndex),
...
func getValuesAsync(m *genericps.GetValuesAsyncMsg) {
	var err error
	err = ps5000aGetValuesAsync(m.Handle(), uint32(m.StartIndex),
...
func getValuesBulk(m *genericps.GetValuesBulkMsg) {
	numOfSamples, err := ps5000aGetValuesBulk(m.Handle(),
		uint32(m.ReqNumOfSamples), uint32(m.FromSegmentIndex), uint32(m.ToSegmentIndex),
...
func getTimebase(m *genericps.GetTimebaseMsg) {
	timeIntervalNanoseconds, maxSamples, err := ps5000aGetTimebase(m.Handle(),
		uint32(m.TimeBase), int32(m.NumOfSamples), m.OverSample, m.SegmentIndex)
...
func getTimebase2(m *genericps.GetTimebase2Msg) {
	timeIntervalNanoseconds, maxSamples, err := ps5000aGetTimebase2(m.Handle(),
...
func setDataBuffer(m *genericps.SetDataBufferMsg) {
	...
	err = ps5000aSetDataBuffer(m.Handle(), ChannelId(m.Ch), m.BufferIn,
...
func getTriggerTimeOffset(m *genericps.GetTriggerTimeOffsetMsg) {
	timeUpper, timeLower, timeUnits, err :=
		ps5000aGetTriggerTimeOffset(m.Handle(), uint32(m.SegmentIndex))
...
func getTriggerTimeOffset64(m *genericps.GetTriggerTimeOffset64Msg) {
	time, timeUnits, err := ps5000aGetTriggerTimeOffset64(m.Handle(), uint32(m.SegmentIndex))
