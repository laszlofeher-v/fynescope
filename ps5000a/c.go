...
// #cgo linux CFLAGS: -g -Wall -I/opt/picoscope/include/libps5000a
...
func ps5000aLpDataReadyGo(handle int16, status int, noOfSamples uint32, overflow int16, pParameter unsafe.Pointer) {
...
func ps5000aLpBlockReadyGo(handle int16, status int, noOfSamples uint32, overflow int16, pParameter unsafe.Pointer) {
...
func ps5000aSetTriggerChannelProperties(handle int16, channelProperties []TriggerChannelProperties, auxOutputEnable bool,
	...
	if len(channelProperties) > 0 {
		...
		for i := range channelProperties {
			...
			cTriggerChannelProperties[i].thresholdMode = (C.PS5000A_THRESHOLD_MODE)(channelProperties[i].ThresholdMode)
		}
	}
