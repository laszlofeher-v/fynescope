...
func getValues(m *genericps.GetValuesMsg) {
	...
	numOfSamples, overflow, err = ps5000GetValues(m.Handle(), m.StartIndex,
		m.ReqNumOfSamples,
		m.DownSampleRatio,
...
func setChannel(m *genericps.SetChannelMsg) {
	var err error
	err = ps5000SetChannel(m.Handle(), ChannelId(m.Channel),
...
func flashLed(m *genericps.FlashLedMsg) {
	err := ps5000FlashLed(m.Handle(), m.Start)
...
func getUnitInfo(m *genericps.GetUnitInfoMsg) {
	...
	s, err = ps5000GetUnitInfo(m.Handle(), PicoInfo(m.Info))
	if err != nil {
...
func getValuesAsync(m *genericps.GetValuesAsyncMsg) {
	var err error
	err = ps5000GetValuesAsync(m.Handle(), m.StartIndex,
		m.NumOfSamples,
		m.DownSampleRatio,
		RatioMode(m.DownSampleRatioMode),
...
func getValuesBulk(m *genericps.GetValuesBulkMsg) {
	...
	numOfSamples, err = ps5000GetValuesBulk(m.Handle(), m.ReqNumOfSamples, m.FromSegmentIndex, m.ToSegmentIndex,
...
func getValuesOverlapped(m *genericps.GetValuesOverlappedMsg) {
	...
	numOfSamples, err = ps5000GetValuesOverlapped(m.Handle(), m.StartIndex, m.ReqNumOfSamples,
...
func getValuesOverlappedBulk(m *genericps.GetValuesOverlappedBulkMsg) {
	...
	numOfSamples, err = ps5000GetValuesOverlappedBulk(m.Handle(), m.StartIndex, m.ReqNumOfSamples,
...
func getAnalogueOffsetMsg(m *genericps.GetAnalogueOffsetMsg) {
	...
	maximumVoltage, minimumVoltage, err = ps5000GetAnalogueOffset(m.Handle(), m.VoltageRange, Coupling(m.Coupling))
...
func getChannelInformation(m *genericps.GetChannelInformationMsg) {
	...
	lengthOfRanges, err = ps5000GetChannelInformation(m.Handle(), m.Info, m.Probe, m.Ranges, ChannelId(m.Channel))
...
func getMaxDownSampleRatio(m *genericps.GetMaxDownSampleRatioMsg) {
	...
	maxDownSampleRatio, err = ps5000GetMaxDownSampleRatio(m.Handle(), m.NumOfUnaggregatedSamples, RatioMode(m.DownSampleRatioMode), m.SegmentIndex)
...
func getMaxSegments(m *genericps.GetMaxSegmentsMsg) {
	maxSegments, err := ps5000GetMaxSegments(m.Handle())
...
func getNumberOfCaptures(m *genericps.GetNumOfCapturesMsg) {
	numOfCaptures, err := ps5000GetNumOfCaptures(m.Handle())
...
func getNumberOfProcessedCaptures(m *genericps.GetNumOfProcessedCapturesMsg) {
	numOfCaptures, err := ps5000GetNumOfProcessedCaptures(m.Handle())
...
func getStreamingLatestValues(m *genericps.GetStreamingLatestValuesMsg) {
	...
	err = ps5000GetStreamingLatestValues(m.Handle(), StreamingReady(m.LpStreamingReadyGoPar), m.Param)
...
func getTimebase(m *genericps.GetTimebaseMsg) {
	timeIntervalNanoseconds, maxSamples, err := ps5000GetTimebase(m.Handle(), m.TimeBase, m.NumOfSamples, m.OverSample, m.SegmentIndex)
...
func getTimebase2(m *genericps.GetTimebase2Msg) {
	timeIntervalNanoseconds, maxSamples, err := ps5000GetTimebase2(m.Handle(), m.TimeBase, m.NumOfSamples, m.OverSample, m.SegmentIndex)
...
func maximumValue(m *genericps.MaximumValueMsg) {
	value, err := ps5000MaximumValue(m.Handle())
...
func minimumValue(m *genericps.MinimumValueMsg) {
	value, err := ps5000MinimumValue(m.Handle())
...
func setSimpleTrigger(m *genericps.SetSimpleTriggerMsg) {
	...
	err = ps5000SetSimpleTrigger(m.Handle(), m.Enable, ChannelId(m.Source),
...
func setDataBuffer(m *genericps.SetDataBufferMsg) {
	...
	err = ps5000SetDataBuffer(m.Handle(), ChannelId(m.Ch), m.BufferIn, m.SegmentIndex, RatioMode(m.Mode))
...
func setDataBuffers(m *genericps.SetDataBuffersMsg) {
	...
	err = ps5000SetDataBuffers(m.Handle(), ChannelId(m.Ch), m.BufferMax, m.BufferMin, m.SegmentIndex, RatioMode(m.Mode))
...
func setUnscaledDataBuffers(m *genericps.SetUnscaledDataBuffersMsg) {
	...
	err = ps5000SetUnscaledDataBuffers(m.Handle(), ChannelId(m.Ch), m.BufferMax, m.BufferMin, m.SegmentIndex, RatioMode(m.Mode))
...
func setEtsTimeBuffer(m *genericps.SetEtsTimeBufferMsg) {
	err := ps5000SetEtsTimeBuffer(m.Handle(), m.Buffer)
...
func setEtsTimeBuffers(m *genericps.SetEtsTimeBuffersMsg) {
	err := ps5000SetEtsTimeBuffers(m.Handle(), m.TimeUpper, m.TimeLower)
	response := m.Rsp().(*genericps.SetEtsTimeBufferRsp)
	response.SetStatus(err)
	m.RspCh() <- struct{}{}
}
func setEts(m *genericps.SetEtsMsg) {
	...
	sampleTimePicoseconds, err = ps5000SetEts(m.Handle(), EtsMode(m.Mode), m.EtsCycles, m.EtsInterleave)
...
func runStreaming(m *genericps.RunStreamingMsg) {
	...
	sampleInterval, err = ps5000RunStreaming(m.Handle(), m.ReqSampleInterval, TimeUnits(m.SampleIntervalTimeUnits), m.MaxPostTriggerSamples,
...
func runBlock(m *genericps.RunBlockMsg) {
	...
	timeIndisposedMs, err = ps5000RunBlock(m.Handle(), m.NumOfPreTriggerSamples, m.NumOfPostTriggerSamples,
...
func setTriggerChannelProperties(m *genericps.SetTriggerChannelPropertiesMsg) {
	...
	err = ps5000SetTriggerChannelProperties(m.Handle(), tcp, m.AuxOutputEnable, m.AutoTriggerMs)
...
func setTriggerChannelConditions(m *genericps.SetTriggerChannelConditionsMsg) {
	...
	err = ps5000SetTriggerChannelConditions(m.Handle(), tc)
...
func setTriggerChannelDirections(m *genericps.SetTriggerChannelDirectionsMsg) {
	...
	err = ps5000SetTriggerChannelDirections(m.Handle(), ThresholdDirection(m.ChannelA),
...
func setTriggerDelay(m *genericps.SetTriggerDelayMsg) {
	err := ps5000SetTriggerDelay(m.Handle(), m.Delay)
...
func setPulseWidthQualifier(m *genericps.SetPulseWidthQualifierMsg) {
	...
	err = ps5000SetPulseWidthQualifier(m.Handle(), c, ThresholdDirection(m.Direction), m.Lower, m.Upper, PulseWidthType(m.PwType))
...
func setTriggerDigitalPortProperties(m *genericps.SetTriggerDigitalPortPropertiesMsg) {
	...
	err = ps5000SetTriggerDigitalPortProperties(m.Handle(), tdp)
...
func setSigGenBuiltInV2(m *genericps.SetSigGenBuiltInV2Msg) {
	...
	err = ps5000SetSigGenBuiltInV2(m.Handle(), m.OffsetVoltage, m.PkToPK, WaveTypeEnum(m.WaveType), m.StartFrequency,
		m.StopFrequency, m.Increment, m.DwellTime, SweepTypeEnum(m.SweepType),
		ExtraOperations(m.Operation), m.Shots, m.Sweeps, SigGenTrigType(m.TriggerType),
		SigGenTrigSource(m.TriggerSource), m.ExtInThreshold)
...
func sigGenFrequencyToPhase(m *genericps.SigGenFrequencyToPhasenMsg) {
	...
	phase, err = ps5000SigGenFrequencyToPhase(m.Handle(), m.Frequency, IndexMode(m.IndexMode), m.BufferLength)
...
func getTriggerTimeOffset(m *genericps.GetTriggerTimeOffsetMsg) {
	...
	timeUpper, timeLower, timeUnits, err = ps5000GetTriggerTimeOffset(m.Handle(), m.SegmentIndex)
...
func getTriggerTimeOffset64(m *genericps.GetTriggerTimeOffset64Msg) {
	time, timeUnits, err := ps5000GetTriggerTimeOffset64(m.Handle(), m.SegmentIndex)
...
func holdOff(m *genericps.HoldOffMsg) {
	err := ps5000HoldOff(m.Handle(), m.HoldOff, HoldOffType(m.HoldOffType))
...
func lsReady(m *genericps.LsReadyMsg) {
	ready, err := ps5000LsReady(m.Handle())
...
func triggerOrPulseWidthQualifierEnabled(m *genericps.TriggerOrPulseWidthQualifierEnabledMsg) {
	triggerEnabled, pulseWidthQualifierEnabledint16, err := ps5000TriggerOrPulseWidthQualifierEnabled(m.Handle())
...
func memorySegments(m *genericps.MemorySegmentsMsg) {
	nMaxSamples, err := ps5000MemorySegments(m.Handle(), m.NSegments)
...
func numOfStreamingValues(m *genericps.NumOfStreamingValuesMsg) {
	numOfValues, err := ps5000NoOfStreamingValues(m.Handle())
...
func queryOutputEdgeDetect(m *genericps.QueryOutputEdgeDetectMsg) {
	state, err := ps5000QueryOutputEdgeDetect(m.Handle())
...
func setDigitalPort(m *genericps.SetDigitalPortMsg) {
	...
	err = ps5000SetDigitalPort(m.Handle(), DigitalPort(m.Port), m.Enabled, m.Logiclevel)
	response := m.Rsp().(*genericps.SetDigitalAnalogTriggerOperandRsp)
	response.SetStatus(err)
	m.RspCh() <- struct{}{}
}
...
func setOutputEdgeDetect(m *genericps.SetOutputEdgeDetectMsg) {
	err := ps5000SetOutputEdgeDetect(m.Handle(), m.State)
...
func setPulseWidthDigitalPortProperties(m *genericps.SetPulseWidthDigitalPortPropertiesMsg) {
	...
	err = ps5000SetPulseWidthDigitalPortProperties(m.Handle(), dd)
...
func setSigGenPropertiesArbitrary(m *genericps.SetSigGenPropertiesArbitraryMsg) {
	err := ps5000SetSigGenPropertiesArbitrary(m.Handle(), m.OffsetVoltage, m.StartDeltaPhase,
...
func setSigGenPropertiesBuiltIn(m *genericps.SetSigGenPropertiesBuiltInMsg) {
	err := ps5000SetSigGenPropertiesBuiltIn(m.Handle(), m.OffsetVoltage, float64(m.StartFrequency),
...
func sigGenArbitraryMinMaxValues(m *genericps.SigGenArbitraryMinMaxValuesMsg) {
	minArbitraryWaveformValue, maxArbitraryWaveformValue, minArbitraryWaveformSize,
		maxArbitraryWaveformSize, err := ps5000SigGenArbitraryMinMaxValues(m.Handle())
...
func sigGenSoftwareControl(m *genericps.SigGenSoftwareControlMsg) {
	err := ps5000SigGenSoftwareControl(m.Handle(), m.State)
