...
func (tp *intervalTriggerPointViewer) draw() {
	...
	if tp.scp.triggerSource != dontCare {
		...
		yList := []float32{}
		if tp.scp.triggerSettingMsg.Type == control.RiseFall || channel.Trigger.Type == settings.TriggerTypeRiseFall {
		...
		} else if tp.scp.triggerSettingMsg.Type == control.WindowPulseWidth || tp.scp.triggerSettingMsg.Type == control.WindowDropout {
			// For Window Pulse Width and Window Dropout, the user wants arrows at BOTH thresholds
			_, yUpper := tp.timeMv2xy(channel.Trigger.Mv)
			_, yLower := tp.timeMv2xy(channel.Trigger.LowerMv)
