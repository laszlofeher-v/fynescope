...
var (
	_ mouser     = (*runtTriggerPointViewer)(nil)
	_ dragger    = (*runtTriggerPointViewer)(nil)
	_ scroller   = (*runtTriggerPointViewer)(nil)
	_ drawer     = (*runtTriggerPointViewer)(nil)
	_ cursorable = (*runtTriggerPointViewer)(nil)
)

func (tp *runtTriggerPointViewer) cursor(x, y float32) (desktop.Cursor, bool) {
...
func (tp *runtTriggerPointViewer) mouseAtUpperHysteresisPoint(x, y float32) bool {
	p := image.Point{X: int(math.Round(float64(x))), Y: int(math.Round(float64(y)))}
	return p.In(tp.uhImgRect)
}

func (tp *runtTriggerPointViewer) mouseAtLowerPoint(x, y float32) bool {
	p := image.Point{X: int(math.Round(float64(x))), Y: int(math.Round(float64(y)))}
	return p.In(tp.lImgRect)
}

func (tp *runtTriggerPointViewer) mouseAtLowerHysteresisPoint(x, y float32) bool {
	p := image.Point{X: int(math.Round(float64(x))), Y: int(math.Round(float64(y)))}
	return p.In(tp.lhImgRect)
}

func (tp *runtTriggerPointViewer) mouseMoved(x, y float32) {
	if tp.scp.inStreamMode() {
		return
	}
	prevMain := tp.mouseAt
...
func (tp *runtTriggerPointViewer) mouseDown(button desktop.MouseButton, modifier fyne.KeyModifier, x, y float32) {
	if tp.scp.inStreamMode() {
		return
	}
	if tp.mouseAt {
...
func (tp *runtTriggerPointViewer) mouseUp(button desktop.MouseButton, modifier fyne.KeyModifier, x, y float32) {
	if tp.scp.inStreamMode() {
		return
	}
	refresh := false
...
func (tp *runtTriggerPointViewer) setLowerDispOffset(dx, x, y float32) {
...
func (tp *runtTriggerPointViewer) dragged(dx, dy, x, y float32) {
	...
	if tp.uhSelected {
		...
		case genericps.TriggerRising, genericps.TriggerInside, genericps.TriggerOutside, genericps.TriggerEnter, genericps.TriggerEnterOrExit, genericps.TriggerPositiveRunt:
			if newH >= channel.Trigger.Mv {
				channel.Trigger.Hysteresis = newH - channel.Trigger.Mv
			}
		case genericps.TriggerFalling, genericps.TriggerExit, genericps.TriggerNegativeRunt:
			if newH <= channel.Trigger.Mv {
...
func (tp *runtTriggerPointViewer) setUpperHysteresisDispOffset(dyh float32) {
	bounds := tp.signalScreen().Bounds()
	h := float64(bounds.Dy())
	if tp.scp.triggerSource < 0 || int(tp.scp.triggerSource) >= len(tp.scp.Settings.Channels) {
		return
...
func (tp *runtTriggerPointViewer) setLowerHysteresisDispOffset(dyh float32) {
	bounds := tp.signalScreen().Bounds()
	h := float64(bounds.Dy())
	if tp.scp.triggerSource < 0 || int(tp.scp.triggerSource) >= len(tp.scp.Settings.Channels) {
		return
...
func (tp *runtTriggerPointViewer) scrolled(delta, x, y float32) {
	if tp.scp.inStreamMode() {
		return
	}
...
func (tp *runtTriggerPointViewer) draw() {
	...
	if tp.scp.triggerSource != dontCare {
		...
		if channel.Trigger.TriggerDirection == genericps.TriggerFalling || channel.Trigger.TriggerDirection == genericps.TriggerExit || channel.Trigger.TriggerDirection == genericps.TriggerNegativeRunt {
			_, yh = tp.timeMv2xy(channel.Trigger.Mv - channel.Trigger.Hysteresis)
		}
		if yh > maxY {
			yh = maxY
...
func newRuntTriggerPointViewer(img rasterImage, scp *ScpDesc, isTimeZoom bool) *runtTriggerPointViewer {
	...
	tp := &runtTriggerPointViewer{triggerPointViewer: triggerPointViewer{rasterPartition: rasterPartition{img: img, imgRect: imgRect}, scp: scp, isTimeZoom: isTimeZoom}}
	return tp
}
