...
// windowDropoutTriggerPointViewer combines:
...
var (
	_ mouser     = (*windowDropoutTriggerPointViewer)(nil)
	_ dragger    = (*windowDropoutTriggerPointViewer)(nil)
	_ scroller   = (*windowDropoutTriggerPointViewer)(nil)
	_ drawer     = (*windowDropoutTriggerPointViewer)(nil)
	_ cursorable = (*windowDropoutTriggerPointViewer)(nil)
)

func newWindowDropoutTriggerPointViewer(img rasterImage, scp *ScpDesc, isTimeZoom bool) *windowDropoutTriggerPointViewer {
	tp := &windowDropoutTriggerPointViewer{
...
func (tp *windowDropoutTriggerPointViewer) mouseAtLowerPoint(x, y float32) bool {
...
func (tp *windowDropoutTriggerPointViewer) mouseAtLowerHysteresisPoint(x, y float32) bool {
	p := image.Point{X: int(math.Round(float64(x))), Y: int(math.Round(float64(y)))}
...
func (tp *windowDropoutTriggerPointViewer) cursor(x, y float32) (desktop.Cursor, bool) {
...
func (tp *windowDropoutTriggerPointViewer) mouseMoved(x, y float32) {
	if tp.scp.inStreamMode() {
		return
	}
...
func (tp *windowDropoutTriggerPointViewer) mouseDown(button desktop.MouseButton, modifier fyne.KeyModifier, x, y float32) {
	if tp.scp.inStreamMode() {
		return
	}
...
func (tp *windowDropoutTriggerPointViewer) mouseUp(button desktop.MouseButton, modifier fyne.KeyModifier, x, y float32) {
	if tp.scp.inStreamMode() {
		return
	}
...
func (tp *windowDropoutTriggerPointViewer) setLowerDispOffset(dx, x, y float32) {
...
func (tp *windowDropoutTriggerPointViewer) setLowerHysteresisDispOffset(dyh float32) {
	bounds := tp.signalScreen().Bounds()
	h := float64(bounds.Dy())
...
func (tp *windowDropoutTriggerPointViewer) dragged(dx, dy, x, y float32) {
	if tp.scp.inStreamMode() {
		return
	}
	if tp.scp.triggerSource < 0 || int(tp.scp.triggerSource) >= len(tp.scp.Settings.Channels) {
...
func (tp *windowDropoutTriggerPointViewer) scrolled(delta, x, y float32) {
	if tp.scp.inStreamMode() {
		return
	}
	switch {
