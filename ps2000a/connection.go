...
func getValuesBulk(m *genericps.GetValuesBulkMsg) {
	...
	numOfSamples, err = ps2000aGetValuesBulk(m.Handle(), uint32(m.ReqNumOfSamples),
...
func getTimebase(m *genericps.GetTimebaseMsg) {
	timeIntervalNanoseconds, maxSamples, err := ps2000aGetTimebase(m.Handle(),
...
func getTimebase2(m *genericps.GetTimebase2Msg) {
	timeIntervalNanoseconds, maxSamples, err := ps2000aGetTimebase2(m.Handle(),
...
func setDataBuffer(m *genericps.SetDataBufferMsg) {
	...
	err = ps2000aSetDataBuffer(m.Handle(), ChannelId(m.Ch), m.BufferIn,
...
func runBlock(m *genericps.RunBlockMsg) {
	...
	timeIndisposedMs, err = ps2000aRunBlock(m.Handle(), int32(m.NumOfPreTriggerSamples),
...
func getTriggerTimeOffset(m *genericps.GetTriggerTimeOffsetMsg) {
	...
	timeUpper, timeLower, timeUnits, err = ps2000aGetTriggerTimeOffset(m.Handle(), uint32(m.SegmentIndex))
