...
func TestEtsBlockMode_DigitalOnlyWaitsForAnalog(t *testing.T) {
	con := genericps.NewConnection()
	handle, err := genericps.OpenDemo(con, genericps.DemoId)
	con.Handle = handle
	if err != nil {
	...
	// Simulate digital port enabled, but NO analog channels enabled
	...
	// Since only digital port is enabled, etsBlockMode must wait in start() and NOT call getTimeBase
	time.Sleep(50 * time.Millisecond)
	select {
	case <-done:
		t.Fatal("etsBlockMode should not have exited or crashed with only digital ports enabled")
	default:
	}

	// Now enable analog channel A
	chA := settings.ChSettings{
		ID:      genericps.ChannelId(0),
		Enabled: true,
	}
	psControl.SetChannelCh <- &chA

	// Give it time to prepare and run
	time.Sleep(100 * time.Millisecond)
	...
	case <-done:
	case <-time.After(2 * time.Second):
		t.Fatal("etsBlockMode did not stop in time after enabling analog channel")
	}
}

func TestEtsBlockModeHugeScopeScreenWidth(t *testing.T) {
	con := genericps.NewConnection()
	handle, err := genericps.OpenDemo(con, genericps.DemoId)
	con.Handle = handle
	if err != nil {
	...
	assert.Empty(t, fatalLogged, "etsBlockMode should not fail with fatal error on huge screen width")
	assert.LessOrEqual(t, psControl.SampleCountRequired, uint64(250000))
	assert.Greater(t, psControl.SampleCountRequired, uint64(0))
}

func TestEtsBlockModeZeroScopeScreenWidth(t *testing.T) {
	con := genericps.NewConnection()
	handle, err := genericps.OpenDemo(con, genericps.DemoId)
	con.Handle = handle
	if err != nil {
	...
	assert.Empty(t, fatalLogged, "etsBlockMode should not fail with fatal error on zero screen width")
	assert.Greater(t, psControl.SampleCountRequired, uint64(0))
}

func TestEtsBlockModeMaxScreenTimeSampleLimit(t *testing.T) {
	con := genericps.NewConnection()
	handle, err := genericps.OpenDemo(con, genericps.DemoId)
	con.Handle = handle
	if err != nil {
	...
	assert.Empty(t, fatalLogged, "etsBlockMode should not fail with fatal error on large maxScreenTime")
	assert.LessOrEqual(t, psControl.SampleCountRequired, uint64(250000))
	assert.Greater(t, psControl.SampleCountRequired, uint64(0))
}
