...
func ps4000aLpDataReadyGo(handle int16, status int, noOfSamples uint32, overflow int16, pParameter unsafe.Pointer) {
...
func ps4000aLpBlockReadyGo(handle int16, status int, noOfSamples uint32, overflow int16, pParameter unsafe.Pointer) {
...
func ps4000aSetTriggerChannelProperties(handle int16, channelProperties []TriggerChannelProperties, auxOutputEnable bool,
	...
	if len(channelProperties) > 0 {
		...
		for i := range channelProperties {
			...
			cTriggerChannelProperties[i].thresholdMode = (C.PS4000A_THRESHOLD_MODE)(channelProperties[i].ThresholdMode)
		}
	}
...
func ps4000aMemorySegments(handle int16, nSegments uint64) (nMaxSamples int64, err error) {
	slog.Debug("ps4000aMemorySegments", "handle", handle, "nSegments", nSegments)
	var maxSamples C.int32_t
	stat := C.ps4000aMemorySegments((C.short)(handle), (C.uint32_t)(nSegments), &maxSamples)
	if stat != C.PICO_OK {
