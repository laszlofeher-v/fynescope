...
func enumerateUnits(bufferLen int16) (count int16, serials string, serialLth int16, err error) {
	...
	go func() {
		...
		serialLth = bufferLen
		slog.Debug("ps3000aEnumerateUnits", "bufferLen", bufferLen)
...
func openUnit(serial string, resolution int) (handle int16, err error) {
	...
	slog.Debug("ps3000aOpenUnit", "serial", serial)
	stat := C.ps3000aOpenUnit((*C.short)(&handle), (*C.schar)(p))
	if stat != C.PICO_OK {
...
func openUnitAsync(serial string, resolution int) (status int16, err error) {
	...
	slog.Debug("ps3000aOpenUnitAsync", "serial", serial)
	stat := C.ps3000aOpenUnitAsync((*C.short)(&status), (*C.schar)(p))
	if stat != C.PICO_OK {
...
func openUnitProgress() (handle int16, progressPercent int16, complete int16, err error) {
	stat := C.ps3000aOpenUnitProgress((*C.short)(&handle),
...
func ps3000aCloseUnit(handle int16) (err error) {
	slog.Debug("ps3000aCloseUnit", "handle", handle)
	stat := C.ps3000aCloseUnit((C.short)(handle))
	if stat != C.PICO_OK {
...
func ps3000aGetUnitInfo(handle int16, info PicoInfo) (infoString string, err error) {
	const listLen = 4096
	var cstrPtr *C.schar
	cstrPtr = (*C.schar)(C.malloc(C.sizeof_schar * listLen))
	defer C.free(unsafe.Pointer(cstrPtr))
	requiredSize := int16(listLen)
	slog.Debug("ps3000aGetUnitInfo", "handle", handle, "info", info)
	stat := C.ps3000aGetUnitInfo((C.short)(handle), cstrPtr, (C.short)(requiredSize),
...
func ps3000aFlashLed(handle int16, start int16) (err error) {
	slog.Debug("ps3000aFlashLed", "handle", handle, "start", start)
	stat := C.ps3000aFlashLed((C.short)(handle), (C.short)(start))
	if stat != C.PICO_OK {
...
func ps3000aGetValuesAsync(handle int16, startIndex, noOfSamples, downSampleRatio uint32,
	...
	slog.Debug("ps3000aGetValuesAsync", "handle", handle, "startIndex", startIndex, "noOfSamples", noOfSamples, "downSampleRatio", downSampleRatio, "downSampleRatioMode", downSampleRatioMode, "lpDataReadyGoPar", lpDataReadyGoPar, "segmentIndex", segmentIndex, "param", param)
	stat := C.ps3000aGetValuesAsync((C.short)(handle),
		(C.uint)(startIndex),
		(C.uint)(noOfSamples),
...
func ps3000aGetValues(handle int16, startIndex, reqNoOfSamples, downSampleRatio uint32,
	downSampleRatioMode RatioMode, segmentIndex uint32) (noOfSamples uint32, overflow int16, err error) {
	slog.Debug("ps3000aGetValues", "handle", handle, "startIndex", startIndex, "reqNoOfSamples", reqNoOfSamples, "downSampleRatio", downSampleRatio, "downSampleRatioMode", downSampleRatioMode, "segmentIndex", segmentIndex)
	stat := C.ps3000aGetValues((C.short)(handle),
		(C.uint)(startIndex),
...
func ps3000aGetValuesBulk(handle int16, reqNoOfSamples uint32, fromSegmentIndex, toSegmentIndex, downSampleRatio uint32,
	downSampleRatioMode RatioMode, overflow []int16) (noSamples uint32, err error) {
	slog.Debug("ps3000aGetValuesBulk", "handle", handle, "reqNoOfSamples", reqNoOfSamples, "fromSegmentIndex", fromSegmentIndex, "toSegmentIndex", toSegmentIndex, "downSampleRatio", downSampleRatio, "downSampleRatioMode", downSampleRatioMode, "overflow", overflow)
	stat := C.ps3000aGetValuesBulk((C.short)(handle),
		(*C.uint)(&reqNoOfSamples),
...
func ps3000aGetValuesOverlapped(handle int16, startIndex, reqNoOfSamples, downSampleRatio uint32,
	downSampleRatioMode RatioMode, segmentIndex uint32, overflow []int16) (noSamples uint32, err error) {
	slog.Debug("ps3000aGetValuesOverlapped", "handle", handle, "startIndex", startIndex, "reqNoOfSamples", reqNoOfSamples, "downSampleRatio", downSampleRatio, "downSampleRatioMode", downSampleRatioMode, "segmentIndex", segmentIndex, "overflow", overflow)
	stat := C.ps3000aGetValuesOverlapped((C.short)(handle),
		(C.uint)(startIndex),
		(*C.uint)(&reqNoOfSamples),
		(C.uint)(downSampleRatio),
		(C.PS3000A_RATIO_MODE)(downSampleRatioMode),
...
func ps3000aGetValuesOverlappedBulk(handle int16, startIndex, reqNoOfSamples, downSampleRatio uint32,
	downSampleRatioMode RatioMode, fromSegmentIndex, toSegmentIndex uint32, overflow []int16) (noSamples uint32, err error) {
	slog.Debug("ps3000aGetValuesOverlappedBulk", "handle", handle, "startIndex", startIndex, "reqNoOfSamples", reqNoOfSamples, "downSampleRatio", downSampleRatio, "downSampleRatioMode", downSampleRatioMode, "fromSegmentIndex", fromSegmentIndex, "toSegmentIndex", toSegmentIndex, "overflow", overflow)
	stat := C.ps3000aGetValuesOverlappedBulk((C.short)(handle),
		(C.uint)(startIndex),
		(*C.uint)(&reqNoOfSamples),
		(C.uint)(downSampleRatio),
		(C.PS3000A_RATIO_MODE)(downSampleRatioMode),
...
func ps3000aGetAnalogueOffset(handle int16, voltageRange int, coupling Coupling) (maximumVoltage, minimumVoltage float32, err error) {
	slog.Debug("ps3000aGetAnalogueOffset", "handle", handle, "voltageRange", voltageRange, "coupling", coupling)
	stat := C.ps3000aGetAnalogueOffset((C.short)(handle),
...
func ps3000aGetChannelInformation(handle int16, info int16, probe int32, ranges []int32, channels ChannelId) (lengthOfRanges int32, err error) {
	lengthOfRanges = int32(len(ranges))
	slog.Debug("ps3000aGetChannelInformation", "handle", handle, "info", info, "probe", probe, "ranges", ranges, "channels", channels)
	stat := C.ps3000aGetChannelInformation((C.short)(handle), (C.PS3000A_CHANNEL_INFO)(info),
...
func ps3000aGetMaxDownSampleRatio(handle int16, noOfUnaggregatedSamples uint32, downSampleRatioMode RatioMode, segmentIndex int32) (maxDownSampleRatio uint32, err error) {
	slog.Debug("ps3000aGetMaxDownSampleRatio", "handle", handle, "noOfUnaggregatedSamples", noOfUnaggregatedSamples, "downSampleRatioMode", downSampleRatioMode, "segmentIndex", segmentIndex)
	stat := C.ps3000aGetMaxDownSampleRatio((C.short)(handle), (C.uint)(noOfUnaggregatedSamples),
...
func ps3000aGetMaxSegments(handle int16) (maxSegments uint32, err error) {
	slog.Debug("ps3000aGetMaxSegments", "handle", handle)
	stat := C.ps3000aGetMaxSegments((C.short)(handle), (*C.uint)(&maxSegments))
	if stat != C.PICO_OK {
...
func ps3000aGetNumOfProcessedCaptures(handle int16) (nCaptures uint32, err error) {
	slog.Debug("ps3000aGetNoOfProcessedCaptures", "handle", handle)
	stat := C.ps3000aGetNoOfProcessedCaptures((C.short)(handle), (*C.uint)(&nCaptures))
	if stat != C.PICO_OK {
...
func ps3000aGetStreamingLatestValues(handle int16, lpStreamingReadyGoPar StreamingReady, param interface{}) (err error) {
...
func ps3000aGetTimebase2(handle int16, timeBase uint32, numOfSamples int32,
	overSample int16, segmentIndex uint32) (timeIntervalNanoseconds float32, maxSamples int32, err error) {
	slog.Debug("ps3000aGetTimebase2", "handle", handle, "timeBase", timeBase, "numOfSamples", numOfSamples, "overSample", overSample, "segmentIndex", segmentIndex)
	stat := C.ps3000aGetTimebase2((C.short)(handle), (C.uint)(timeBase), (C.int)(numOfSamples),
...
func ps3000aSetChannel(handle int16, channel ChannelId, enabled bool, couplingType Coupling, voltageRange RangeEnum, analogOffset float32) (err error) {
	slog.Debug("ps3000aSetChannel", "handle", handle, "channel", channel, "enabled", enabled, "couplingType", couplingType, "voltageRange", voltageRange, "analogOffset", analogOffset)
	stat := C.ps3000aSetChannel((C.short)(handle), (C.PS3000A_CHANNEL)(channel), (C.short)(boolToint16(enabled)),
...
func ps3000aMaximumValue(handle int16) (value int32, err error) {
	slog.Debug("ps3000aMaximumValue", "handle", handle)
	var val int16
	stat := C.ps3000aMaximumValue((C.short)(handle), (*C.short)(&val))
	value = int32(val)
	if stat != C.PICO_OK {
...
func ps3000aMinimumValue(handle int16) (value int32, err error) {
	slog.Debug("ps3000aMinimumValue", "handle", handle)
	var val int16
	stat := C.ps3000aMinimumValue((C.short)(handle), (*C.short)(&val))
	value = int32(val)
	if stat != C.PICO_OK {
...
func ps3000aSetSimpleTrigger(handle int16, enable bool, source ChannelId, threshold int16,
	direction ThresholdDirection, delay uint32, autoTriggerMs int16) (err error) {
	slog.Debug("ps3000aSetSimpleTrigger", "handle", handle, "enable", enable, "src", source, "threshold", threshold, "direction", direction, "delay", delay, "autoTriggerMs", autoTriggerMs)
	stat := C.ps3000aSetSimpleTrigger((C.short)(handle), (C.short)(boolToint16(enable)),
...
func ps3000aSetDataBuffer(handle int16, ch ChannelId, bufferIn []int16, segmentIndex uint32,
	mode RatioMode) (err error) {

	slog.Debug("ps3000aSetDataBuffer", "handle", handle, "ch", ch, "segmentIndex", segmentIndex, "mode", mode)
	stat := C.ps3000aSetDataBuffer((C.short)(handle), (C.PS3000A_CHANNEL)(ch), (*C.short)(&bufferIn[0]),
...
func ps3000aSetDataBuffers(handle int16, ch ChannelId, bufferMax, bufferMin []int16, segmentIndex uint32, mode RatioMode) (err error) {
	slog.Debug("ps3000aSetDataBuffers", "handle", handle, "ch", ch, "segmentIndex", segmentIndex, "mode", mode)
	stat := C.ps3000aSetDataBuffers((C.short)(handle), (C.PS3000A_CHANNEL)(ch), (*C.short)(&bufferMax[0]),
...
func ps3000aSetUnscaledDataBuffers(handle int16, ch ChannelId, bufferMax, bufferMin []int16, segmentIndex uint32, mode RatioMode) (err error) {
	slog.Debug("ps3000aSetUnscaledDataBuffers", "handle", handle, "ch", ch, "segmentIndex", segmentIndex, "mode", mode)
	stat := C.ps3000aSetDataBuffers((C.short)(handle), (C.PS3000A_CHANNEL)(ch), (*C.short)(&bufferMax[0]),
		(*C.short)(&bufferMin[0]), (C.int)(len(bufferMax)), (C.uint)(segmentIndex),
		(C.PS3000A_RATIO_MODE)(mode))
	if stat != C.PICO_OK {
...
func ps3000aSetEtsTimeBuffer(handle int16, buffer []int64) (err error) {
	slog.Debug("ps3000aSetEtsTimeBuffer", "handle", handle, "buffer", buffer)
	stat := C.ps3000aSetEtsTimeBuffer((C.short)(handle), (*C.long)(&buffer[0]),
		(C.int)(len(buffer)))
	if stat != C.PICO_OK {
...
func ps3000aSetEtsTimeBuffers(handle int16, timeUpper, timeLower []uint32) (err error) {
	slog.Debug("ps3000aSetEtsTimeBuffers", "handle", handle, "timeUpper", timeUpper, "timeLower", timeLower)
	stat := C.ps3000aSetEtsTimeBuffers((C.short)(handle), (*C.uint)(&timeUpper[0]),
...
func ps3000aSetEts(handle int16, mode EtsMode, etsCycles int16, etsInterLeave int16) (sampleTimePicoseconds int32, err error) {
	slog.Debug("ps3000aSetEts", "handle", handle, "mode", mode, "etsCycles", etsCycles, "etsInterLeave", etsInterLeave)
	stat := C.ps3000aSetEts((C.short)(handle), (C.PS3000A_ETS_MODE)(mode),
...
func ps3000aRunStreaming(handle int16, reqSampleInterval uint32, sampleIntervalTimeUnits TimeUnits,
	...
	slog.Debug("ps3000aRunStreaming", "handle", handle, "reqSampleInterval", reqSampleInterval, "sampleIntervalTimeUnits", sampleIntervalTimeUnits, "maxPreTriggerSamples", maxPreTriggerSamples, "maxPostTriggerSamples", maxPostTriggerSamples, "autoStop", autoStop, "downSampleRatio", downSampleRatio, "downSampleRatioMode", downSampleRatioMode, "overviewBufferSize", overviewBufferSize)
	stat := C.ps3000aRunStreaming((C.short)(handle), (*C.uint)(&reqSampleInterval),
...
func ps3000aRunBlock(handle int16, noOfPreTriggerSamples, noOfPostTriggerSamples int32,
	...
	slog.Debug("ps3000aRunBlock", "handle", handle, "noOfPreTriggerSamples", noOfPreTriggerSamples, "noOfPostTriggerSamples", noOfPostTriggerSamples, "timeBase", timeBase, "overSample", overSample, "segmentIndex", segmentIndex, "lpBlockReadyGoPar", lpBlockReadyGoPar, "param", param)
...
func ps3000aSetTriggerChannelProperties(handle int16, channelProperties []TriggerChannelProperties, auxOutputEnable bool,
	...
	if len(channelProperties) > 0 {
		...
		for i := range channelProperties {
			...
			cTriggerChannelProperties[i].thresholdMode = (C.PS3000A_THRESHOLD_MODE)(channelProperties[i].ThresholdMode)
		}
	}
	...
	slog.Debug("ps3000aSetTriggerChannelProperties", "handle", handle, "channelProperties", channelProperties, "auxOutputEnable", auxOutputEnable, "autoTriggerMs", autoTriggerMs)
	stat := C.ps3000aSetTriggerChannelProperties((C.short)(handle),
...
func ps3000aSetTriggerChannelConditions(handle int16, triggerConditions []TriggerConditions) (err error) {
	...
	slog.Debug("ps3000aSetTriggerChannelConditions", "handle", handle, "triggerConditions", triggerConditions)
	stat := C.ps3000aSetTriggerChannelConditions((C.short)(handle),
...
func ps3000aSetTriggerChannelDirections(handle int16, channelA, channelB, channelC, channelD, ext, aux ThresholdDirection) (err error) {
	slog.Debug("ps3000aSetTriggerChannelDirections", "handle", handle, "channelA", channelA, "channelB", channelB, "channelC", channelC, "channelD", channelD, "ext", ext, "aux", aux)
	stat := C.ps3000aSetTriggerChannelDirections((C.short)(handle),
...
func ps3000aSetTriggerDelay(handle int16, delay uint32) (err error) {
	slog.Debug("ps3000aSetTriggerDelay", "handle", handle, "delay", delay)
	stat := C.ps3000aSetTriggerDelay((C.short)(handle), (C.uint)(delay))
	if stat != C.PICO_OK {
...
func ps3000aSetPulseWidthQualifier(handle int16, conditions []PwqConditions, direction ThresholdDirection, lower, upper uint32,
...
func ps3000aSetTriggerDigitalPortProperties(handle int16, digitalDirections []DigitalChannelDirections) (err error) {
...
func ps3000aStop(handle int16) (err error) {
	slog.Debug("ps3000aStop", "handle", handle)
	stat := C.ps3000aStop((C.short)(handle))
	if stat != C.PICO_OK {
...
func ps3000aSetSigGenBuiltIn(handle int16, offsetVoltage int32, pkToPK uint32, waveType WaveTypeEnum,
	...
	slog.Debug("ps3000aSetSigGenBuiltIn", "handle", handle, "offsetVoltage", offsetVoltage, "pkToPK", pkToPK, "waveType", waveType, "startFrequency", startFrequency, "stopFrequency", stopFrequency, "increment", increment, "dwellTime", dwellTime, "sweepType", sweepType, "operation", operation, "shots", shots, "sweeps", sweeps, "triggerType", triggerType, "triggerSource", triggerSource, "extInThreshold", extInThreshold)
	stat := C.ps3000aSetSigGenBuiltIn((C.short)(handle), (C.int)(offsetVoltage),
...
func ps3000aSetSigGenBuiltInV2(handle int16, offsetVoltage int32, pkToPK uint32, waveType WaveTypeEnum,
	startFrequency, stopFrequency, increment, dwellTime float64, sweepType SweepTypeEnum,
	operation ExtraOperations, shots, sweeps uint32, triggerType SigGenTrigType,
	triggerSource SigGenTrigSource, extInThreshold int16) (err error) {
	slog.Debug("ps3000aSetSigGenBuiltInV2", "handle", handle, "offsetVoltage", offsetVoltage, "pkToPK", pkToPK, "waveType", waveType, "startFrequency", startFrequency, "stopFrequency", stopFrequency, "increment", increment, "dwellTime", dwellTime, "sweepType", sweepType, "operation", operation, "shots", shots, "sweeps", sweeps, "triggerType", triggerType, "triggerSource", triggerSource, "extInThreshold", extInThreshold)
	stat := C.ps3000aSetSigGenBuiltInV2((C.short)(handle), (C.int)(offsetVoltage),
...
func ps3000aSigGenFrequencyToPhase(handle int16, frequency float64, indexMode IndexMode, bufferLength uint32) (phase uint32, err error) {
	slog.Debug("ps3000aSigGenFrequencyToPhase", "handle", handle, "frequency", frequency, "indexMode", indexMode, "bufferLength", bufferLength)
	stat := C.ps3000aSigGenFrequencyToPhase((C.short)(handle), (C.double)(frequency),
...
func ps3000aSetNoCaptures(handle int16, nCaptures uint32) (err error) {
	slog.Debug("ps3000aSetNoOfCaptures", "handle", handle, "nCaptures", nCaptures)
	stat := C.ps3000aSetNoOfCaptures((C.short)(handle), (C.uint)(nCaptures))
	if stat != C.PICO_OK {
...
func ps3000aGetTriggerTimeOffset(handle int16, segmentIndex uint32) (timeUpper, timeLower uint32, timeUnits TimeUnits, err error) {
	slog.Debug("ps3000aGetTriggerTimeOffset", "handle", handle, "segmentIndex", segmentIndex)
	stat := C.ps3000aGetTriggerTimeOffset((C.short)(handle), (*C.uint)(&timeUpper),
...
func ps3000aGetTriggerTimeOffset64(handle int16, segmentIndex uint32) (time int64, timeUnits TimeUnits, err error) {
	slog.Debug("ps3000aGetTriggerTimeOffset64", "handle", handle, "segmentIndex", segmentIndex)
	stat := C.ps3000aGetTriggerTimeOffset64((C.short)(handle), (*C.long)(&time),
...
func ps3000aGetValuesTriggerTimeOffsetBulk(handle int16, timesUpper, timesLower []uint32, timeUnits []TimeUnits,
	fromSegmentIndex, toSegmentIndex uint32) (err error) {
	slog.Debug("ps3000aGetValuesTriggerTimeOffsetBulk", "handle", handle, "timesUpper", timesUpper, "timesLower", timesLower, "timeUnits", timeUnits, "fromSegmentIndex", fromSegmentIndex, "toSegmentIndex", toSegmentIndex)
	stat := C.ps3000aGetValuesTriggerTimeOffsetBulk((C.short)(handle), (*C.uint)(&timesUpper[0]),
...
func ps3000aGetValuesTriggerTimeOffsetBulk64(handle int16, times []int64, timeUnits []TimeUnits,
	fromSegmentIndex, toSegmentIndex uint32) (err error) {
	slog.Debug("ps3000aGetValuesTriggerTimeOffsetBulk64", "handle", handle, "times", times, "timeUnits", timeUnits, "fromSegmentIndex", fromSegmentIndex, "toSegmentIndex", toSegmentIndex)
	stat := C.ps3000aGetValuesTriggerTimeOffsetBulk64((C.short)(handle), (*C.long)(&times[0]),
...
func ps3000aHoldOff(handle int16, holdOff uint64, holdOffType HoldOffType) (err error) {
	slog.Debug("ps3000aHoldOff", "handle", handle, "holdOff", holdOff, "holdOffType", holdOffType)
	stat := C.ps3000aHoldOff((C.short)(handle), (C.ulong)(holdOff), (C.PS3000A_HOLDOFF_TYPE)(holdOffType))
	if stat != C.PICO_OK {
...
func ps3000aLsReady(handle int16) (ready int16, err error) {
	slog.Debug("ps3000aLsReady", "handle", handle)
	stat := C.ps3000aIsReady((C.short)(handle), (*C.short)(&ready))
	if stat != C.PICO_OK {
...
func ps3000aTriggerOrPulseWidthQualifierEnabled(handle int16) (triggerEnabled, pulseWidthQualifierEnabledint16 int16, err error) {
	slog.Debug("ps3000aTriggerOrPulseWidthQualifierEnabled", "handle", handle)
	stat := C.ps3000aIsTriggerOrPulseWidthQualifierEnabled((C.short)(handle),
...
func ps3000aMemorySegments(handle int16, nSegments uint64) (nMaxSamples int64, err error) {
	slog.Debug("ps3000aMemorySegments", "handle", handle, "nSegments", nSegments)
	var maxSamples C.int32_t
	stat := C.ps3000aMemorySegments((C.short)(handle), (C.uint32_t)(nSegments), &maxSamples)
	if stat != C.PICO_OK {
...
func ps3000aNoOfStreamingValues(handle int16) (noOfValues uint32, err error) {
	slog.Debug("ps3000aNoOfStreamingValues", "handle", handle)
	stat := C.ps3000aNoOfStreamingValues((C.short)(handle),
		(*C.uint)(&noOfValues))
	if stat != C.PICO_OK {
...
func ps3000aPingUnit(handle int16) (err error) {
	slog.Debug("ps3000aPingUnit", "handle", handle)
	stat := C.ps3000aPingUnit((C.short)(handle))
	if stat != C.PICO_OK {
...
func ps3000aQueryOutputEdgeDetect(handle int16) (state int16, err error) {
	slog.Debug("ps3000aQueryOutputEdgeDetect", "handle", handle)
	stat := C.ps3000aQueryOutputEdgeDetect((C.short)(handle),
		(*C.short)(&state))
	if stat != C.PICO_OK {
...
func ps3000aSetDigitalPort(handle int16, port DigitalPort, enabled bool, logiclevel int16) (err error) {
	slog.Debug("ps3000aSetDigitalPort", "handle", handle, "port", port, "enabled", enabled, "logiclevel", logiclevel)
	stat := C.ps3000aSetDigitalPort((C.short)(handle),
...
func ps3000aSetOutputEdgeDetect(handle int16, state int16) (err error) {
	slog.Debug("ps3000aSetOutputEdgeDetect", "handle", handle, "state", state)
	stat := C.ps3000aSetOutputEdgeDetect((C.short)(handle), (C.short)(state))
	if stat != C.PICO_OK {
...
func ps3000aSetSigGenArbitrary(handle int16, offsetVoltage int32, pkToPK uint32,
	...
	triggerSource SigGenTrigSource, extInThreshold int16) (err error) {
	slog.Debug("ps3000aSetSigGenArbitrary", "handle", handle, "offsetVoltage", offsetVoltage, "pkToPK", pkToPK, "startDeltaPhase", startDeltaPhase, "stopDeltaPhase", stopDeltaPhase, "deltaPhaseIncrement", deltaPhaseIncrement, "dwellCount", dwellCount, "arbitraryWaveform", arbitraryWaveform, "sweepType", sweepType, "operation", operation, "indexMode", indexMode, "shots", shots, "sweeps", sweeps, "triggerType", triggerType, "triggerSource", triggerSource, "extInThreshold", extInThreshold)
	stat := C.ps3000aSetSigGenArbitrary((C.short)(handle), (C.int)(offsetVoltage),
...
func ps3000aSetSigGenPropertiesArbitrary(handle int16, offsetVoltage int32,
	startDeltaPhase, stopDeltaPhase, deltaPhaseIncrement, dwellCount uint32,
	sweepType SweepTypeEnum, operation ExtraOperations,
	indexMode IndexMode, shots, sweeps uint32, triggerType SigGenTrigType,
	triggerSource SigGenTrigSource, extInThreshold int16) (err error) {
	slog.Debug("ps3000aSetSigGenPropertiesArbitrary", "handle", handle, "offsetVoltage", offsetVoltage, "startDeltaPhase", startDeltaPhase, "stopDeltaPhase", stopDeltaPhase, "deltaPhaseIncrement", deltaPhaseIncrement, "dwellCount", dwellCount, "sweepType", sweepType, "operation", operation, "indexMode", indexMode, "shots", shots, "sweeps", sweeps, "triggerType", triggerType, "triggerSource", triggerSource, "extInThreshold", extInThreshold)
	stat := C.ps3000aSetSigGenPropertiesArbitrary((C.short)(handle),
...
func ps3000aSetSigGenPropertiesBuiltIn(handle int16, offsetVoltage int32,
	...
	triggerSource SigGenTrigSource, extInThreshold int16) (err error) {
	slog.Debug("ps3000aSetSigGenPropertiesBuiltIn", "handle", handle, "offsetVoltage", offsetVoltage, "startFrequency", startFrequency, "stopFrequency", stopFrequency, "increment", increment, "dwellTime", dwellTime, "sweepType", sweepType, "shots", shots, "sweeps", sweeps, "triggerType", triggerType, "triggerSource", triggerSource, "extInThreshold", extInThreshold)
	stat := C.ps3000aSetSigGenPropertiesBuiltIn((C.short)(handle),
...
func ps3000aSigGenArbitraryMinMaxValues(handle int16) (minArbitraryWaveformValue, maxArbitraryWaveformValue int16,
	minArbitraryWaveformSize, maxArbitraryWaveformSize uint32, err error) {
	slog.Debug("ps3000aSigGenArbitraryMinMaxValues", "handle", handle)
	stat := C.ps3000aSigGenArbitraryMinMaxValues((C.short)(handle),
...
func ps3000aSigGenSoftwareControl(handle int16, state int16) (err error) {
	slog.Debug("ps3000aSigGenSoftwareControl", "handle", handle, "state", state)
	stat := C.ps3000aSigGenSoftwareControl((C.short)(handle),
		(C.short)(state))
	if stat != C.PICO_OK {
		err = fmt.Errorf("SetSigGenBuiltIn:  %s", psc.StatStr(int(stat)))
	}
