...
func enumerateUnits(bufferLen int16) (count int16, serials string, serialLth int16, err error) {
	// ps3000 doesn't enumerate units natively by returning strings.
	// We'll check if a unit is connected by attempting to open it.
	handle := C.ps3000_open_unit()
	if handle > 0 {
		C.ps3000_close_unit(handle)
		count = 1
		serials = "PS3000_1"
...
func openUnitProgress() (handle int16, progressPercent, complete int16, err error) {
	err = fmt.Errorf("openUnitProgress not supported on ps3000")
	return
}

func ps3000CloseUnit(handle int16) (err error) {
	slog.Debug("ps3000CloseUnit", "handle", handle)
	C.ps3000_close_unit((C.short)(handle))
	return
}

func ps3000GetUnitInfo(handle int16, info PicoInfo) (infoString string, err error) {
	const listLen = 4096
...
func ps3000FlashLed(handle int16, start int16) (err error) {
	slog.Debug("ps3000FlashLed", "handle", handle, "start", start)
	C.ps3000_flash_led((C.short)(handle)) // does not take start on ps3000, or wait does it?
	return
}
...
//export lpStreamingReadyGo3000
func lpStreamingReadyGo3000(overviewBuffers **C.int16_t, overflow C.int16_t, triggeredAt C.uint32_t, triggered C.int16_t, auto_stop C.int16_t, nValues C.uint32_t) {
	if regLpStreamingReadyGo != nil {
		// we can't easily extract data here, but typically streaming handles this differently on ps3000.
...
func ps3000GetStreamingLatestValues(handle int16, lpStreamingReadyGoPar genericps.StreamingReady, param interface{}) (err error) {
	regLpStreamingReadyGo = lpStreamingReadyGoPar
	C.ps3000_get_streaming_last_values((C.short)(handle), (C.GetOverviewBuffersMaxMin)(C.lpStreamingReady3000))
	return
}

func ps3000GetTimebase(handle int16, timeBase uint32, noOfSamples int32, overSample int16, segmentIndex uint32) (timeIntervalNanoseconds, maxSamples int32, err error) {
	slog.Debug("ps3000GetTimebase", "handle", handle, "timeBase", timeBase, "noOfSamples", noOfSamples, "overSample", overSample)
	var timeUnits C.int16_t
	stat := C.ps3000_get_timebase((C.short)(handle), (C.short)(timeBase), (C.int)(noOfSamples),
		(*C.int)(&timeIntervalNanoseconds), &timeUnits, (C.short)(overSample),
		(*C.int)(&maxSamples))
	if stat == 0 {
		err = fmt.Errorf("GetTimebase failed")
	}
	// For ps3000, timeUnits indicates the units of timeInterval. We need to convert it to ns.
...
func ps3000SetChannel(handle int16, channel ChannelId, enabled bool, couplingType Coupling, voltageRange RangeEnum, analogOffset float32) (err error) {
	slog.Debug("ps3000SetChannel", "handle", handle, "channel", channel, "enabled", enabled, "couplingType", couplingType, "voltageRange", voltageRange)
	// ps3000 doesn't have analogOffset
	stat := C.ps3000_set_channel((C.short)(handle), (C.short)(channel), (C.short)(boolToint16(enabled)),
...
func ps3000MaximumValue(handle int16) (value int32, err error) {
	return 32767, nil
}

func ps3000MinimumValue(handle int16) (value int32, err error) {
	return -32767, nil
}

func ps3000SetSimpleTrigger(handle int16, enable bool, source ChannelId, threshold int16,
	direction ThresholdDirection, delay uint32, autoTriggerMs int16) (err error) {
	slog.Debug("ps3000SetSimpleTrigger", "handle", handle, "enable", enable, "src", source, "threshold", threshold, "direction", direction, "delay", delay, "autoTriggerMs", autoTriggerMs)
	var stat C.short
	if !enable {
		stat = C.ps3000_set_trigger((C.short)(handle), 5, 0, 0, 0, 0) // PS3000_NONE=5
	} else {
		stat = C.ps3000_set_trigger((C.short)(handle), (C.short)(source), (C.short)(threshold),
...
func ps3000RunStreaming(handle int16, reqSampleInterval uint32, sampleIntervalTimeUnits TimeUnits,
	...
	slog.Debug("ps3000RunStreaming")
	stat := C.ps3000_run_streaming_ns((C.short)(handle), (C.uint)(reqSampleInterval),
		(C.PS3000_TIME_UNITS)(sampleIntervalTimeUnits), (C.uint)(maxPreTriggerSamples+maxPostTriggerSamples),
...
func ps3000RunBlock(handle int16, noOfPreTriggerSamples, noOfPostTriggerSamples int32,
	...
	go func() {
		for {
			ready := C.ps3000_ready((C.short)(handle))
			if ready > 0 {
...
func ps3000Stop(handle int16) (err error) {
	slog.Debug("ps3000Stop", "handle", handle)
	C.ps3000_stop((C.short)(handle))
	return
}
...
func ps3000SetSigGenBuiltIn(handle int16, offsetVoltage int32, pkToPK uint32, waveType WaveTypeEnum,
...
func ps3000IsReady(handle int16) (ready int16, err error) {
	slog.Debug("ps3000IsReady", "handle", handle)
	r := C.ps3000_ready((C.short)(handle))
	ready = int16(r)
	return
}
