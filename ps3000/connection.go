...
func setChannel(m *genericps.SetChannelMsg) {
	err := ps3000SetChannel(m.Handle(), ChannelId(m.Channel),
...
func flashLed(m *genericps.FlashLedMsg) {
	err := ps3000FlashLed(m.Handle(), m.Start)
...
func getUnitInfo(m *genericps.GetUnitInfoMsg) {
	s, err := ps3000GetUnitInfo(m.Handle(), PicoInfo(m.Info))
	if err != nil {
...
func getStreamingLatestValues(m *genericps.GetStreamingLatestValuesMsg) {
	err := ps3000GetStreamingLatestValues(m.Handle(), genericps.StreamingReady(m.LpStreamingReadyGoPar), m.Param)
...
func maximumValue(m *genericps.MaximumValueMsg) {
	value, err := ps3000MaximumValue(m.Handle())
...
func minimumValue(m *genericps.MinimumValueMsg) {
	value, err := ps3000MinimumValue(m.Handle())
...
func setSimpleTrigger(m *genericps.SetSimpleTriggerMsg) {
	err := ps3000SetSimpleTrigger(m.Handle(), m.Enable, ChannelId(m.Source),
...
func runStreaming(m *genericps.RunStreamingMsg) {
	sampleInterval, err := ps3000RunStreaming(m.Handle(), m.ReqSampleInterval, TimeUnits(m.SampleIntervalTimeUnits), m.MaxPreTriggerSamples,
...
func setSigGenBuiltIn(m *genericps.SetSigGenBuiltInMsg) {
	err := ps3000SetSigGenBuiltIn(m.Handle(), m.OffsetVoltage, m.PkToPK, WaveTypeEnum(m.WaveType), m.StartFrequency,
...
func lsReady(m *genericps.LsReadyMsg) {
	ready, err := ps3000IsReady(m.Handle())
